<template>
    <el-sub-menu :index="getPath(parentPath, item.path)" v-if="item.children && item.children.length > 0">
        <template #title>
            <MoIcon :icon-name="item.meta.icon" v-if="level === 0" class="ml-1 mr-1 text-lg" />
            <span>{{ $t(`route.${item.meta.title}`) }}</span>
        </template>
        <SidebarItem :item="item1" :parent-path="getPath(parentPath, item.path)" :level="level + 1"
